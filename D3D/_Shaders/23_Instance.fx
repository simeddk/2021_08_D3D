...
VertexOutput VS_InstanceID(VertexInput input)
{
    ...
    input.Position = input.Position + float4(input.InstanceID * 2.0f, 0, 0, 1);
...
VertexOutput VS_ColorInstanceID(VertexInput input)
{
    VertexOutput output;
...
float4 PS_ColorInstanceID(VertexOutput input) : SV_Target
{
    float3 normal = normalize(input.Normal);
    float lambert = dot(normal, -LightDirection);
...
technique11 T0
{
    ...
    P_VP(P2, VS_ColorInstanceID, PS_ColorInstanceID)
}